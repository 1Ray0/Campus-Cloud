...
import { FastTemplatesTab, type FastTemplate } from "@/components/Applications/FastTemplatesTab"
...
export function ResourceCreatePage() {
  const { t } = useTranslation(["resources", "validation", "common", "messages", "applications"])
  ...
  const isSubmitReady = useMemo(() => {
    const basicReady = Boolean(
      watchedHostname?.trim() && watchedPassword,
    )
